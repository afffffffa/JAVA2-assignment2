...
public class Controller implements Initializable {
    ...
    public void setCurrentUsername(String num) {
        Platform.runLater(() -> {String members = num;
...
//                                预处理
                                    ...
                                    if (!found) {
                                        ...
                                        Platform.runLater(() -> { ObservableList<ChatItem> items = chatList.getItems();
...
//                                chatItemUsernames[chatItemUsernames.length - 1] = sendBy;
                                    ...
                                    if (!found1) {
                                        ...
                                        Platform.runLater(() -> { chatList.getItems().add(newChatItem);ObservableList<ChatItem> items = chatList.getItems();
//                                            Collections.sort(items, new ChatItemComparator());
                                            chatList.setCellFactory(new ChatItemCellFactory());
                                            chatList.refresh();
                                            chatList.setOnMouseClicked(event -> {
